...
class ArrayList:
    ...
    def __str__(self):
        ...
        for i in range(self.len-1):
            stroong += str(self.__getitem__(i)) + ", "
        stroong += str(self.__getitem__(self.len-1)) + "]"
        return stroong
        ### END SOLUTION
    ...
    def append(self, value):
        ...
        if self.len == len(self.data):
            if(self.len == 0):
                tempConstrainedList = ConstrainedList(1)
                self.data = tempConstrainedList
            else:
                tempConstrainedList = ConstrainedList(self.len*2)
                for i in range(self.len):
                    tempConstrainedList[i] = self.data[i]
                self.data = tempConstrainedList
    ...
    def insert(self, idx, value):
        ...
        if self.len == len(self.data):
            tempConstrainedList = ConstrainedList(self.len*2-1)
            for i in range(self.len):
                tempConstrainedList[i] = self.data[i]
            self.data = tempConstrainedList
    ...
    def remove(self, value):
        ...
        if self.__contains__(value):
            self.__delitem__(self.index(value, 0))
        else:
            raise ValueError
        ### END SOLUTION

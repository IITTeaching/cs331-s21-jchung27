...
class SuffixArray():
    ...
    def __init__(self, document: str):
        ...
        self.document= document
        for i in range(len(document)):
            self.suffix_arr.append(document[i:])
        
        compare_thing = lambda x,y:  0 if x == y else (-1 if x < y else 1)
        self.suffix_arr = mysort(self.suffix_arr, compare_thing)
        for i in range(len(self.suffix_arr)):
            self.suffix_arr[i] = len(document) - len(self.suffix_arr[i])
    ...
    def positions(self, searchstr: str):
        ...
        return [mybinsearch(self.suffix_arr, searchstr, compare)]
    ...
    def contains(self, searchstr: str):
        ...
        """
        compare = lambda x, y: 0 if self.document[x:x + len(searchstr)] == searchstr else (-1 if self.document[x:x + len(searchstr)] < searchstr else 1)
        return mybinsearch(self.suffix_arr, searchstr, compare) != -1
